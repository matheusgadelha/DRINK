...
/**
* Function to access pixel intesity of the point p relative to the keypoint kp in the image img.
* @param kp Central KeyPoint
* @param p Point relative to KeyPoint kp
* @param img Image where the pixel intensity will be returned.
*/
inline
...
std::vector< int > DRINK::result_statistics;
std::vector< int > DRINK::patternSizes;
std::vector< std::vector<int> > DRINK::pair_result_statistics;
std::vector<DRINK::TestPair> DRINK::pairs;
std::vector<DRINK::TestPair> DRINK::allPairsVec;
std::vector<float> DRINK::pair_std_dev;
std::vector< std::vector<unsigned char> > DRINK::data;

DRINK::PatternPoint::PatternPoint(int _x, int _y) {
    x = _x;
    y = _y;
    sigma = 1;
}

DRINK::PatternPoint::PatternPoint() {
    x = 0;
    y = 0;
    sigma = 1;
}

DRINK::TestPair::TestPair(int _a, int _b) {
    a = _a;
    b = _b;
    resultCount.resize(5);
}

DRINK::TestPair::TestPair() {
    a = 0;
    b = 0;
}

float DRINK::l2Distance(PatternPoint a, PatternPoint b) {
...
void DRINK::selectPairs(float _delta_min, float _delta_max)
{
    pairs.clear();
...
void DRINK::increaseStatistics(const std::bitset<RBITS> r) const
{
...
void DRINK::increaseStatisticsForPair(const std::bitset<RBITS> r, int p, int kp) const
{
...
int DRINK::descriptorSize() const
{
    const int DRINK_type_size = (numBits * pairs.size()) / 8;
    return DRINK_type_size;
}

int DRINK::descriptorType() const
{
    return CV_8UC1;
}

void DRINK::computeImpl( const Mat& image, std::vector<KeyPoint>& keypoints, Mat& DRINKs) const
{

    Mat sum;
    Mat grayImage = image;
    std::vector<Mat> channels;
    std::vector<Mat> channels_integral;

    if ( color )
    {
        if( image.type() == CV_8U )
        {
            std::cout << "ERROR: Single channel image cannot be processed as a multi-channel one\n";
            return ;
        }
        ...
        {
            std::cout << "ERROR: Non supported image type for color processing\n";
        }
    }
    else
    {
    ...
    if( color )
    {
        ...
        {
            integral( channels[i], channels_integral[i], CV_32S );
        }
    }
    else
    {
        integral(grayImage, sum, CV_32S);
        channels_integral.push_back( sum );
    }
    ...
    for (size_t i_kp = keypoints.size(); i_kp--;)
    {
        ...
        if (keypoints[i_kp].pt.x <= patternSizes[kpScales[i_kp]] ||
    ...
    if( color )
        DRINKs = Mat::zeros( (int) keypoints.size(), 3*DRINK_type_size, CV_8U );
    else
        DRINKs = Mat::zeros( (int) keypoints.size(), DRINK_type_size, CV_8U );

    int channel_num = color ? 3 : 1;

    // Iterates over all keypoints
    for (unsigned int i_kp = 0; i_kp < keypoints.size(); ++i_kp)
    {
        uchar* desc = DRINKs.ptr(i_kp);
        ...
        for (int ch = 0; ch < channel_num; ++ch)
        {
            for (size_t i = 0; i < pairs.size(); i++)
            {
				...
				std::cout << channels[ch].cols << std::endl;
				std::cout << channels_integral[ch].cols << std::endl;
				std::cout << channels[ch].rows << std::endl;
				std::cout << channels_integral[ch].rows << std::endl;
